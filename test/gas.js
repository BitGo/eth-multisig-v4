...
const createAndGetWallet = async (creator, signers) => {
  ...
  const walletFactory = await WalletFactory.deploy(await walletImplementation.getAddress());
...
describe('Wallet Operations Gas Usage', function () {
  ...
  before(async () => {
    ...
    [deployer, signer1, signer2, user1, destinationAccount] = await ethers.getSigners();
  ...
  it('WalletSimple send [ @skip-on-coverage ]', async function () {
    ...
    const tx = wallet.connect(deployer).sendMultiSig(
        destinationAccount.address,
        amount,
        data,
        expireTime,
  ...
  it('WalletSimple send batch [ @skip-on-coverage ]', async function () {
    ...
    const recipients = [deployer.address, destinationAccount.address, user1.address];
    const amounts = [ethers.parseEther('1'), ethers.parseEther('2'), ethers.parseEther('3')];

    // Fund the wallet with enough ether for the batch
    await user1.sendTransaction({ to: await wallet.getAddress(), value: amounts[0] + amounts[1] + amounts[2] });
