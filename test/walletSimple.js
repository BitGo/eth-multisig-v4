...
describe('WalletSimple Contracts', function () {
  ...
  before(async () => {
    ...
    [deployer, signer1, signer2, user1, user2, user3, user4, user5] =
  ...
  const createWallet = async (creator, signers) => {
    const WalletSimpleImplementation = await ethers.getContractFactory(
      'WalletSimple'
    );
    ...
    ({
      ...
      describe(`${walletContractName}`, function () {
        ...
        const createWallet = async (creator, signers) => {
          ...
          return createWalletHelper(
            WalletSimpleImplementation,
            creator,
            signers
          );
        };
        ...
        describe('Wallet creation', function () {
          ...
          it('Reverts when creating with 0 address signer', async function () {
            ...
            const walletImplementation =
              await WalletSimpleImplementation.deploy([]);
            await walletImplementation.waitForDeployment();
            await expect(
              walletImplementation.init([deployer.address, ethers.ZeroAddress])
        ...
        describe('NFT Support', function () {
          ...
          describe('ERC1155', function () {
            it('Should receive ERC1155 tokens', async function () {
              ...
              expect(
                await nft.balanceOf(await wallet.getAddress(), id)
              ).to.equal(amt);
            });
          });
        });
        ...
        describe('ERC165 Interface Support', function () {
          ...
          it('Should support ERC165 interface detection', async function () {
            ...
            try {
              const supported = await wallet.supportsInterface(
                ERC165_INTERFACE_ID
              );
...
describe('Recover address from signature', function () {
  ...
  beforeEach(async function () {
    ...
    wallet = await createWalletHelper(WalletSimpleImplementation, deployer, [
      deployer.address,
      signer1.address,
      signer2.address
    ]);
  ...
  it('Matches util.ecsign signer via Transacted event (10 iterations)', async function () {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      const parsed = wallet.interface.parseLog(log);
      expect(parsed.args.otherSigner.toLowerCase()).to.equal(
