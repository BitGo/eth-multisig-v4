...
describe('Batcher', () => {
  ...
  let Batcher, Reentry, Fail, GasGuzzler, GasHeavy, FixedSupplyToken, TestBatcherDriver;
  let batcher, reentry, fail, gasGuzzler, gasHeavy, fixedSupplyToken;
  let deployer, batcherOwner, tokenOwner, sender, recipient1, recipient2, recipient3, otherAccount;

  // --- Helper function to generate random BigInt values ---
  const randBigInt = (max) => BigInt(Math.floor(Math.random() * max) + 1);
  const createRandBigIntArr = (len) => Array.from({ length: len }, () => randBigInt(100));

  before(async () => {
    // Get signers from Hardhat
    [deployer, sender, recipient1, recipient2, recipient3, otherAccount, batcherOwner, tokenOwner] = await ethers.getSigners();
  ...
  afterEach(async () => {
    if (batcher) {
      ...
      if (balance > 0) {
        ...
        await batcher.connect(batcherOwner).recover(deployer.address, balance, '0x');
      }
    }
  });

  // --- Fix: Re-introduce the TestBatcherDriver helper function ---
  const runTestBatcherDriver = async ({ recipients, values, extraValue = 0n, expectRevert = false, expectedErrorMsg = '' }) => {
    const testBatcherDriver = await TestBatcherDriver.deploy(await batcher.getAddress(), false, false);
    ...
    const txPromise = testBatcherDriver.connect(sender).driveTest(recipientAddresses, values, { value: sendVal });
    ...
    if (expectRevert) {
      if (expectedErrorMsg) {
        ...
        if ([
          ...
          await expect(txPromise).to.be.revertedWithCustomError(batcher, expectedErrorMsg);
        } else {
  ...
  describe('Good transactions', () => {
    ...
    it('Correctly sends to multiple recipients with exact amount', async () => {
      await runTestBatcherDriver({
        ...
        values: createRandBigIntArr(3).map(v => ethers.parseEther(v.toString()))
      });
    });
    ...
    it('Correctly sends with duplicate recipients', async () => {
      ...
      const totalValue = values.reduce((sum, v) => sum + v, 0n);
      const tx = () => testBatcherDriver.connect(sender).driveTest(recipients.map(r => r.address), values, { value: totalValue });
  ...
  describe('Failed transactions', () => {
    it('Reverts if total sent out does not equal total received', async () => {
      console.log('DEBUG: Starting test - Reverts if total sent out does not equal total received');
      try {
        await runTestBatcherDriver({
          recipients: [recipient1],
          values: [ethers.parseEther('1')],
    ...
    it('Reverts with empty recipients list', async () => {
      ...
      try {
        const testBatcherDriver = await TestBatcherDriver.deploy(await batcher.getAddress(), false, false);
        await testBatcherDriver.waitForDeployment();
        const txPromise = testBatcherDriver.connect(sender).driveTest([], [], { value: 0 });
        await expect(txPromise).to.be.revertedWithCustomError(batcher, 'EmptyRecipientsList');
      } catch (err) {
        console.log('DEBUG: Error thrown:', err);
        throw err;
      }
    ...
    it('Reverts with mismatched recipients and values arrays', async () => {
      console.log('DEBUG: Starting test - Reverts with mismatched recipients and values arrays');
      try {
        const testBatcherDriver = await TestBatcherDriver.deploy(await batcher.getAddress(), false, false);
        await testBatcherDriver.waitForDeployment();
        const txPromise = testBatcherDriver.connect(sender).driveTest([recipient1.address, recipient2.address], [5n], { value: 5n });
        await expect(txPromise).to.be.revertedWithCustomError(batcher, 'UnequalRecipientsAndValues');
      } catch (err) {
        console.log('DEBUG: Error thrown:', err);
        throw err;
      }
    ...
    it('Reverts when recipients length exceeds limit', async () => {
      console.log('DEBUG: Starting test - Reverts when recipients length exceeds limit');
      const limit = 256;
      ...
      try {
        const testBatcherDriver = await TestBatcherDriver.deploy(await batcher.getAddress(), false, false);
        await testBatcherDriver.waitForDeployment();
        const txPromise = testBatcherDriver.connect(sender).driveTest(recipients, values, { value: BigInt(values.length) });
        await expect(txPromise).to.be.revertedWithCustomError(batcher, 'TooManyRecipients');
      } catch (err) {
        console.log('DEBUG: Error thrown:', err);
        throw err;
      }
  ...
  describe('Only owner functions', () => {
    it('Successfully transfers ownership', async () => {
      await batcher.connect(batcherOwner).transferOwnership(otherAccount.address);
    ...
    it('Successfully sets transfer gas limit', async () => {
      console.log('DEBUG: Starting test - Successfully sets transfer gas limit');
      try {
    ...
    it('Fails to set transfer gas limit below minimum', async () => {
      console.log('DEBUG: Starting test - Fails to set transfer gas limit below minimum');
      try {
        await expect(
  ...
  describe('Batch ERC20 Token Transfers', () => {
    ...
    it('Successfully transfers tokens to multiple recipients', async () => {
      ...
      await fixedSupplyToken.connect(tokenOwner).transfer(sender.address, 1000n);
      await fixedSupplyToken.connect(sender).approve(await batcher.getAddress(), 1000n);

      const tx = () => batcher.connect(sender).batchTransferFrom(
    ...
    it('Successfully transfers tokens with duplicate recipients', async () => {
      const recipients = [recipient1.address, recipient1.address, recipient2.address];
    ...
    it('Fails when sender has insufficient token balance', async () => {
      ...
      await fixedSupplyToken.connect(sender).approve(await batcher.getAddress(), 1100n);

      await expect(
        batcher.connect(sender).batchTransferFrom(
          fixedSupplyToken.getAddress(),
          recipients,
          amounts
        )
    ...
    it('Fails when sender has not approved enough tokens', async () => {
      ...
      await fixedSupplyToken.connect(tokenOwner).transfer(sender.address, 1000n);
      await fixedSupplyToken.connect(sender).approve(await batcher.getAddress(), 400n); // Only approved 400
  ...
  describe('Additional Stress Tests and Edge Cases', () => {
    ...
    it('Handles complex reentry scenarios correctly', async () => {
      ...
      await expect(
        batcher.connect(sender).batch(
          [await reentry.getAddress()], 
          [value], 
          { value: value }
        )
  ...
  describe('Recovery Functions for Stuck Tokens', () => {
    ...
    it('Successfully recovers stuck ERC20 tokens', async () => {
      ...
      await fixedSupplyToken.connect(tokenOwner).transfer(await batcher.getAddress(), stuckAmount);
      
      // Create recovery call data for transferring tokens back to owner
      const transferCalldata = fixedSupplyToken.interface.encodeFunctionData('transfer', [
        batcherOwner.address,
        stuckAmount
      ]);

      await expect(
        batcher.connect(batcherOwner).recover(
          await fixedSupplyToken.getAddress(),
          0,
          transferCalldata
        )
    ...
    it('Fails when non-owner tries to use recovery function', async () => {
      ...
      await expect(
        batcher.connect(sender).recover(
          await fixedSupplyToken.getAddress(),
          0,
          transferCalldata
        )
