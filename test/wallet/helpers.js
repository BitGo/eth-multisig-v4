...
const createForwarderFromWallet = async (wallet, autoFlush = true) => {
  ...
  return {
    ...
    create: async () =>
      executeCreateForwarder(forwarderFactory, inputSalt, parent, autoFlush)
  };
};
...
const createRecoveryWalletFactory = async () => {
  const RecoveryWalletSimple = await ethers.getContractFactory(
    'RecoveryWalletSimple'
  );
  const walletContract = await RecoveryWalletSimple.deploy([]);
  await walletContract.waitForDeployment();
  const RecoveryWalletFactory = await ethers.getContractFactory(
    'RecoveryWalletFactory'
  );
  const walletFactory = await RecoveryWalletFactory.deploy(
    await walletContract.getAddress()
  );
  await walletFactory.waitForDeployment();
  return {
