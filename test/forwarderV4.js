...
describe('ForwarderV4', function () {
  ...
  const createForwarder = async (creator, parentSigner, feeSigner, flush721 = true, flush1155 = true) => {
    const forwarder = await ForwarderV4.connect(creator).deploy([]);
    await forwarder.waitForDeployment();
    await forwarder.connect(creator).init(parentSigner.address, feeSigner.address, flush721, flush1155);
    return forwarder;
  };

  before(async () => {
    // Get signers
    [deployer, owner, parent, feeAddress, user1, user2, user3, user4, user5] = await ethers.getSigners();
  ...
  describe('Core Functionality', () => {
    ...
    it('Should not init twice', async function () {
      await expect(
        forwarder.connect(owner).init(parent.address, feeAddress.address, true, true)
    ...
    it('Should not init if parent address is invalid', async function () {
        ...
        await expect(f.init(ethers.ZeroAddress, feeAddress.address, true, true)).to.be.revertedWith('Invalid parent address');
    });

    it('Should not init if fee address is invalid', async function () {
        const f = await ForwarderV4.deploy([]);
        await f.waitForDeployment();
        await expect(f.init(parent.address, ethers.ZeroAddress, true, true)).to.be.revertedWith('Invalid fee address');
    });
    ...
    it('should fail to change autoFlush settings if caller is not an allowed address', async () => {
      await expect(forwarder.connect(user1).setAutoFlush721(false)).to.be.revertedWith('Address is not allowed');
      await expect(forwarder.connect(user1).setAutoFlush1155(false)).to.be.revertedWith('Address is not allowed');
    });
    ...
    it('should batch flush ERC20 tokens when called by an allowed address', async () => {
        ...
        const initialForwarderBalance = await ethers.provider.call({
            to: tetherAddress,
            data: tether.interface.encodeFunctionData('balanceOf', [forwarderAddress])
        });
        
        // Execute batch flush
        await forwarder.connect(feeAddress).batchFlushERC20Tokens([tetherAddress]);
  ...
  describe('NFT Support', function () {
    ...
    beforeEach(async () => {
      ...
      await NoFlushForwarderImpl.init(parent.address, feeAddress.address, false, false);
    ...
    it('Should receive ERC721 with auto flush', async function () {
        ...
        await token721.connect(user1).safeTransferFrom(user1.address, await autoFlushForwarder.getAddress(), tokenId);
    ...
    it('Should receive ERC1155 with auto flush', async function () {
        ...
        await token1155.connect(user1).safeTransferFrom(user1.address, await autoFlushForwarder.getAddress(), tokenId, amount, '0x');
        
        expect(await token1155.balanceOf(parent.address, tokenId)).to.equal(amount);
        expect(await token1155.balanceOf(await autoFlushForwarder.getAddress(), tokenId)).to.equal(0);
    });
    ...
    it('Should flush ERC721 when called by an allowed address', async function () {
        ...
        await token721.mint(user1.address, tokenId);
        const forwarderAddress = await noAutoFlushForwarder.getAddress();
        await token721.connect(user1).transferFrom(user1.address, forwarderAddress, tokenId);

        await noAutoFlushForwarder.connect(feeAddress).flushERC721Token(await token721.getAddress(), tokenId);
        expect(await token721.ownerOf(tokenId)).to.equal(parent.address);
    });
    ...
    it('Should flush ERC1155 when called by an allowed address', async function () {
        const tokenId = 1;
        const amount = 50;
        await token1155.mint(await noAutoFlushForwarder.getAddress(), tokenId, amount, '0x');

        await noAutoFlushForwarder.connect(parent).flushERC1155Tokens(await token1155.getAddress(), tokenId);
        expect(await token1155.balanceOf(parent.address, tokenId)).to.equal(amount);
    });
  });
});
