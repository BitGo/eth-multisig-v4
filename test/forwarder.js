...
describe('Forwarder', function () {
  ...
  let forwarder, token721, token1155, reentryForwarder;
  let owner, parent, user1, user2, user3, user4, user5;

  // Helper to create and initialize a forwarder
  const createForwarder = async (creator, parentSigner, flush721 = true, flush1155 = true) => {
    const forwarderContract = await Forwarder.connect(creator).deploy([]);
    await forwarderContract.waitForDeployment();
    await forwarderContract.connect(creator).init(parentSigner.address, flush721, flush1155);
    return forwarderContract;
  };

  before(async () => {
    // Get signers
    [owner, parent, user1, user2, user3, user4, user5] = await ethers.getSigners();
  ...
  describe('Core Functionality', () => {
    it('Basic forwarding test', async function () {
      ...
      await expect(
        user1.sendTransaction({ to: await forwarder.getAddress(), value: amount })
    ...
    it('Flush on initialization', async function () {
      ...
      const forwarderAddress = ethers.getCreateAddress({ from: owner.address, nonce });

      // Send funds to the predicted address before deployment
      await user2.sendTransaction({ to: forwarderAddress, value: amount });
      expect(await ethers.provider.getBalance(forwarderAddress)).to.equal(amount);
    ...
    it('Should not init twice', async function () {
      const forwarder = await createForwarder(owner, parent);
      await expect(forwarder.connect(owner).init(parent.address, true, true)).to.be.reverted;
    });
    ...
    it('should fail to change autoFlush settings if caller is not parent', async () => {
      const forwarder = await createForwarder(owner, parent);
      await expect(forwarder.connect(user1).setAutoFlush721(false)).to.be.reverted;
      await expect(forwarder.connect(user1).setAutoFlush1155(false)).to.be.reverted;
    });
  });
  ...
  describe('NFT Support', function () {
    ...
    describe('ERC721', () => {
      it('Should receive with safeTransferFrom with auto flush', async function () {
        ...
        await token721.connect(user1).safeTransferFrom(user1.address, await forwarder.getAddress(), tokenId);
      ...
      it('Should receive with safeTransferFrom with no auto flush', async function () {
        ...
        await token721.connect(user1).safeTransferFrom(user1.address, forwarderAddress, tokenId);
      ...
      it('Should be able to flush ERC721 tokens when called by parent', async function () {
        ...
        await forwarder.connect(parent).flushERC721Token(await token721.getAddress(), tokenId);
        expect(await token721.ownerOf(tokenId)).to.equal(parent.address);
      });
    });
    ...
    describe('ERC1155', () => {
      it('should receive erc1155 tokens with autoflush on', async function () {
        ...
        await token1155.connect(user1).safeTransferFrom(user1.address, await forwarder.getAddress(), tokenId, amount, '0x');
        expect(await token1155.balanceOf(parent.address, tokenId)).to.equal(amount);
        expect(await token1155.balanceOf(await forwarder.getAddress(), tokenId)).to.equal(0);
      });
      ...
      it('should receive batch erc1155 tokens with autoflush on', async function () {
        ...
        await token1155.connect(user1).safeBatchTransferFrom(user1.address, await forwarder.getAddress(), tokenIds, amounts, '0x');
        expect(await token1155.balanceOf(parent.address, tokenIds[0])).to.equal(amounts[0]);
        expect(await token1155.balanceOf(parent.address, tokenIds[1])).to.equal(amounts[1]);
      });
      ...
      it('should be able to flush erc1155 tokens when called by parent', async () => {
        ...
        const tokenId = 1;
        const amount = 10;
        await token1155.mint(await forwarder.getAddress(), tokenId, amount, '0x');

        await forwarder.connect(parent).flushERC1155Tokens(await token1155.getAddress(), tokenId);
        expect(await token1155.balanceOf(parent.address, tokenId)).to.equal(amount);
      });
    });
  });
  ...
  describe('ERC165', function () {
    it('should reflect actual supportsInterface implementation', async function () {
      ...
      const iERC1155Receiver = await forwarder.supportsInterface(IERC1155ReceiverId);
      console.log('DEBUG: supportsInterface(ERC165):', erc165);
      console.log('DEBUG: supportsInterface(IForwarder):', iForwarder);
      console.log('DEBUG: supportsInterface(IERC1155Receiver):', iERC1155Receiver);
  ...
  describe('Re-Entrancy', function () {
    ...
    it('should fail with reentry set to true for onERC721Received function', async function () {
      ...
      await expect(
        token721.connect(user1).safeTransferFrom(user1.address, reentryForwarderAddress, 1)
      ).to.be.reverted;
    });
    ...
    it('should pass with reentry set to false for onERC721Received function', async function () {
      ...
      await token721.connect(user1).safeTransferFrom(user1.address, reentryForwarderAddress, 1);
    ...
    it('should fail with reentry set to true for onERC1155Received function', async function () {
      ...
      // This should fail due to re-entrancy - transfer to reentryForwarder
      await expect(
        token1155.connect(user1).safeTransferFrom(user1.address, reentryForwarderAddress, 1, 10, '0x')
      ).to.be.reverted;
    });
    ...
    it('should pass with reentry set to false for onERC1155Received function', async function () {
      ...
      // Transfer to reentryForwarder, not forwarder
      await token1155.connect(user1).safeTransferFrom(user1.address, reentryForwarderAddress, 1, 10, '0x');
      
      expect(await token1155.balanceOf(reentryForwarderAddress, 1)).to.equal(10);
    });
    ...
    it('should fail with reentry set to true for onERC1155BatchReceived function', async function () {
      ...
      // This should fail due to re-entrancy - transfer to reentryForwarder
      await expect(
        token1155.connect(user1).safeBatchTransferFrom(user1.address, reentryForwarderAddress, [1, 2], [10, 20], '0x')
      ).to.be.reverted;
    });
    ...
    it('should pass with reentry set to false for onERC1155BatchReceived function', async function () {
      ...
      // Transfer to reentryForwarder, not forwarder
      await token1155.connect(user1).safeBatchTransferFrom(user1.address, reentryForwarderAddress, [1, 2], [10, 20], '0x');
      
      expect(await token1155.balanceOf(reentryForwarderAddress, 1)).to.equal(10);
      expect(await token1155.balanceOf(reentryForwarderAddress, 2)).to.equal(20);
    });
  });
  ...
  describe('Additional NFT Support Tests', () => {
    ...
    describe('ERC721 Advanced Features', () => {
      it('Should support ERC721 safeTransferFrom with data', async function () {
        ...
        const data = '0x1234';
        await token721.connect(user1)['safeTransferFrom(address,address,uint256,bytes)'](
          user1.address, 
      ...
      it('Should fail if token contract does not support ERC721', async function () {
        ...
        await expect(
          forwarder.connect(parent).flushERC721Token(await notERC721.getAddress(), 1)
        ).to.be.revertedWith('The tokenContractAddress does not support the ERC721 interface');
      });
      ...
      it('Should be able to flush ERC721 tokens when forwarder has approval', async function () {
        ...
        await token721.connect(user1)['safeTransferFrom(address,address,uint256)'](
          user1.address,
          await forwarder.getAddress(),
          1
        );
      ...
      it('Should fail to flush ERC721 tokens when forwarder is not owner or approved', async function () {
        ...
        await expect(
          forwarder.connect(parent).flushERC721Token(await token721.getAddress(), 1)
    ...
    describe('ERC1155 Advanced Features', () => {
      it('Should handle multiple ERC1155 token types', async function () {
        ...
        await token1155.connect(user1).safeBatchTransferFrom(
          user1.address, 
          await forwarder.getAddress(), 
          [1, 2, 3], 
          [5, 10, 15], 
      ...
      it('Should batch flush ERC1155 tokens back to parent', async function () {
        ...
        const noFlushForwarder = await createForwarder(owner, parent, true, false);
        ...
        await noFlushForwarder.connect(parent).batchFlushERC1155Tokens(
      ...
      it('Should fail to flush ERC1155 tokens when caller is not parent', async function () {
        ...
        await expect(
          forwarder.connect(user1).batchFlushERC1155Tokens(await token1155.getAddress(), [1])
