import { checkAndEnableBigBlocks, checkBigBlocksStatus } from './checkAndEnableBigBlocks';
...
async function testBigBlocksImplementation() {
  ...
  supportedChains.forEach(chainId => {
    console.log(`  - Chain ${chainId}: ${isBigBlocksSupported(chainId) ? '✅ Supported' : '❌ Not supported'}`);
  });
  ...
  if (!privateKey) {
    ...
    console.log('   Set this environment variable to test actual BigBlocks functionality.');
