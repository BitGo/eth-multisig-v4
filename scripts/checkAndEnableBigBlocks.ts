...
export async function checkAndEnableBigBlocks(
  ...
  try {
    ...
    if (isCurrentlyEnabled) {
      console.log(
        `✅ BigBlocks is already enabled for ${userAddress} on ${config.name}`
      );
      return;
    }

    console.log(
      `🔄 BigBlocks is not enabled. Enabling BigBlocks for ${userAddress} on ${config.name}...`
    );
    ...
    if (isNowEnabled) {
      console.log(
        `✅ BigBlocks successfully enabled for ${userAddress} on ${config.name}`
      );
    } else {
      console.log(
        `⚠️  BigBlocks enablement may not have taken effect immediately for ${userAddress} on ${config.name}`
      );
    }
  } catch (error) {
    console.error(
      `❌ Failed to check/enable BigBlocks on ${config.name}:`,
